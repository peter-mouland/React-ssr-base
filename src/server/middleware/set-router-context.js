...
function getMatch(routesArray, url) {
  return routesArray
    .find((route) => matchPath(url, route.path, { path: route.path, exact: true, strict: false }));
}
...
async function getRouteData(routesArray, url, dispatch) {
  ...
  routesArray
    ...
    .forEach((route) => {
      const match = matchPath(url, route.path, { path: route.path, exact: true, strict: false });
      if (match) {
