...
describe('reducers/index', () => {
  ...
  context('orders reducer', () => {
    it('will always return given state by default', () => {
      expect(reducers.orders(fakeState, {})).to.equal(fakeState, 'State should always be returned');
    });

    it('will return loading state if action is pending', () => {
      const type = `${actions.FETCH_ORDERS}_PENDING`;
      const result = reducers.orders(fakeState, { type });
    ...
    it('will return update state and payload if action is fulfilled', () => {
      const type = `${actions.FETCH_ORDERS}_FULFILLED`;
      const status = chance.integer();
      const payload = { message: chance.sentence() };
      const result = reducers.orders(fakeState, { type, status, payload });
